...
async function register(email: string, name: string) {
  const res = await fetch("https://classcombinator2.vercel.app/api/register", {
    method: "POST",
    headers: {
  ...
  return (
    <div className="container">
      ...
      <div className="sub-title">
        <span>
          <strong>Winter 2026</strong> Registration ahead?
        </span>
      ...
      {isError && (
        <div>
          <p style={{ color: "red" }}>
            Invalid email! Please only use university email to login.
          </p>
        </div>
      )}
